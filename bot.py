...
class MMTicketView(discord.ui.View):
    ...
    async def claim(self, interaction: discord.Interaction, btn: discord.ui.Button):
        ...
        # Find the ticket creator by channel topic (their user ID)
        creator_member = None
        if ch.topic and ch.topic.isdigit():
            creator_member = guild.get_member(int(ch.topic))

        # Deny send_messages for all staff roles that currently have access
        new_overwrites = {
            guild.default_role: discord.PermissionOverwrite(read_messages=False),
        ...
        # All other staff roles: can still READ but NOT send
        for rid in MM_CLAIM:
            r = guild.get_role(rid)
        ...
        embed.description = (
            f"{interaction.user.mention} will be your Middleman for today.\n\n"
            "🔒 This ticket is now **private** — only the Middleman and ticket creator can send messages."
        )
        embed.set_footer(text=FOOTER)
...
class MMModal(discord.ui.Modal, title="Middleman Ticket | BloxExchange"):
...
async def mercy(interaction: discord.Interaction, user: discord.Member):
    ...
    # Ghost ping the scammed user in the designated channel
    ghost_ch = interaction.guild.get_channel(1507543943552962560)
    if ghost_ch:
        ghost_msg = await ghost_ch.send(user.mention)
