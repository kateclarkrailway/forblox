...
PROMOTE_CEILING = {
    ROLE["mid_manager"]:  ROLE["middleman"],
    ROLE["co_founder"]:   ROLE["senior_mid"],
    ROLE["chief_exec"]:   ROLE["co_founder"],
...
SUPPORT_STAFF_ROLE = 1506445268160348221

ALL_STAFF    = list(ROLE.values())
TICKET_STAFF = ALL_STAFF
MM_CLAIM     = ALL_STAFF
INDEX_CLAIM  = [ROLE["index_mm"]] + ALL_STAFF
...
def mm_overwrites(guild, opener):
    ow = {
...
def support_overwrites(guild, opener):
    ow = {
        guild.default_role: discord.PermissionOverwrite(read_messages=False),
        opener: discord.PermissionOverwrite(read_messages=True, send_messages=True),
    }
...
def index_overwrites(guild, opener):
    ow = {
        guild.default_role: discord.PermissionOverwrite(read_messages=False),
        opener: discord.PermissionOverwrite(read_messages=True, send_messages=True),
    }
    for rid in INDEX_CLAIM:
        r = guild.get_role(rid)
        if r:
            ow[r] = discord.PermissionOverwrite(read_messages=True, send_messages=True)
    return ow
...
class IndexTicketView(discord.ui.View):
    ...
    async def claim(self, interaction: discord.Interaction, btn: discord.ui.Button):
        if not has_role(interaction.user, INDEX_CLAIM):
            await interaction.response.send_message("No permission.", ephemeral=True)
            return
        self.claimed_by = interaction.user.mention
        btn.disabled    = True
...
class MMRequestModal(discord.ui.Modal, title="Request Middleman | BloxExchange"):
    who_trading = discord.ui.TextInput(
        label="Who are you trading?",
        placeholder="e.g. @Username or their Roblox username",
        style=discord.TextStyle.short,
        required=True
    )
    what_trade = discord.ui.TextInput(
        label="What is the trade?",
        placeholder="e.g. Frost Dragon for Corrupt + 5 garamas",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.description = (
            f"{self.opener.mention}, Thank you for using our middleman services.\n\n"
            "Please wait for a middleman to assist you.\n\n"
            "If you have any questions, please let a staff member know."
        )
        embed.add_field(name="👤 Trading With", value=str(self.who_trading), inline=False)
        embed.add_field(name="🔄 Trade Details", value=str(self.what_trade), inline=False)
        embed.set_footer(text=FOOTER)
    ...
    async def request(self, interaction: discord.Interaction, btn: discord.ui.Button):
        ...
        for c in cat.channels:
            if c.topic == str(interaction.user.id):
                ...
                return
        modal = MMRequestModal(guild=guild, opener=interaction.user)
...
class SupportModal(discord.ui.Modal, title="Support Ticket | BloxExchange"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="👤 Opened By", value=f"{self.opener.mention} ({self.opener.id})", inline=False)
        embed.add_field(name="📅 Opened At", value=ts_now(), inline=False)
        embed.set_footer(text=FOOTER)
        view = SupportTicketView(creator=self.opener.mention)
        await ch.send(content=f"<@&{SUPPORT_STAFF_ROLE}> {self.opener.mention}", embed=embed, view=view)
...
class IndexBaseSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text=FOOTER)
        pings = " ".join(f"<@&{rid}>" for rid in [ROLE["index_mm"]]) + f" {interaction.user.mention}"
...
# ─── Info Command Views (Discord TOS + Guidelines buttons) ────────────────────
...
async def setup_mm(interaction: discord.Interaction):
    ...
    embed.add_field(name="• Request Middleman",
        value="Read our mm-tos first, then tap **Request Middleman** and fill out the form.", inline=False)
    embed.add_field(name="• Vouch Required",
        value="You must vouch your middleman after the trade in #vouches. Failing to do so within 24 hours results in a **Blacklist** from our MM Service.", inline=False)
    embed.add_field(name="• Troll Tickets",
        value="Creating any form of troll tickets will result in a **Middleman ban**.", inline=False)
    embed.add_field(name="• Disclaimer",
        value="We are **NOT** responsible for anything that happens after the trade is done.", inline=False)
    embed.set_footer(text=FOOTER)
...
async def setup_support(interaction: discord.Interaction):
    ...
    embed.add_field(name="• Create a ticket if you need support for:",
...
async def cmd_rules(interaction: discord.Interaction):
    embed = discord.Embed(title="📋 BloxExchange Marketplace | Rules & Guidelines", color=0x2b2d31)
    embed.add_field(name="1. 📜 Follow Discord ToS and Guidelines",
        value="We're on Discord's platform, therefore we'll automatically follow their regulations. Make sure you don't violate their terms and guidelines.", inline=False)
    embed.add_field(name="2. 🔒 Personal Information",
        value="Do not post personal information about anyone without their consent. Any impersonation within MMs or other members are also not allowed.", inline=False)
    embed.add_field(name="3. ✅ Content Appropriate",
        value="All content should be safe for work and appropriate for the server's community. No NSFW, graphic, or disturbing content.", inline=False)
    embed.add_field(name="4. 📌 Use the Correct Channels",
        value="Post messages, images, and discussions in the appropriate channels. Read channel descriptions and rules to avoid clutter.", inline=False)
    embed.add_field(name="5. 🚫 No Illegal Activities",
        value="Sharing, discussing, or promoting illegal activities is strictly prohibited. This includes piracy, hacking, and any other form of illegal behavior.", inline=False)
    embed.add_field(name="6. 🔐 Respect Privacy",
        value="Do not share personal information (yours or others') without consent. Respect everyone's privacy.", inline=False)
    embed.add_field(name="7. 🎭 No Impersonation",
        value="Do not impersonate other members, including server staff, celebrities, or other users.", inline=False)
    embed.add_field(name="8. 💬 Follow Discord's Terms of Service",
        value="All members must adhere to Discord's Terms of Service and Community Guidelines. https://discord.com/terms", inline=False)
    embed.add_field(name="9. 👂 Listen to Staff",
        value="Staff decisions are final. If you have issues or concerns, contact a staff member privately and respectfully.", inline=False)
    embed.add_field(name="10. ⚠️ We Are NOT Responsible",
        value="We are not responsible when one of our server ad buyers scams you. This means that going first in one of their trades is **YOUR OWN RISK!** If you get scammed by one of them DM an owner and we will take the ad down as fast as possible.", inline=False)
    embed.add_field(name="11. 📢 Server Ads",
        value="We do **NOT** refund purchased server ads. Breaking the server's rules and getting banned will lead to an ad remove with no refunds.", inline=False)
    embed.set_footer(text="BloxExchange Marketplace • Today")
...
async def cmd_about(interaction: discord.Interaction):
    embed = discord.Embed(title="🛡️ BloxExchange — About Us", color=0x2b2d31)
    embed.description = (
...
async def cmd_faq(interaction: discord.Interaction):
    embed = discord.Embed(title="🛡️ BloxExchange Marketplace | FAQ", color=0x2b2d31)
    embed.add_field(name="What is BloxExchange?",
        value="BloxExchange is a platform that provides a secure player-to-player marketplace for buyers and sellers of online gaming products. We provide a system for secure transactions — you do the rest. We have marketplaces for **250+ games** and leading titles!", inline=False)
    embed.add_field(name="How does the Middleman service work?",
        value="Our verified Middlemen act as trusted third parties to hold and transfer items/funds during a trade. This ensures both parties are protected throughout the entire deal.", inline=False)
    embed.add_field(name="Is it free to use?",
        value="Yes! Our Middleman service is completely free for standard trades. Simply open a ticket and request a Middleman.", inline=False)
    embed.add_field(name="How long does a trade take?",
        value="Most trades are completed within minutes. Our Middlemen are available 24/7 to assist you as quickly as possible.", inline=False)
    embed.add_field(name="What if something goes wrong?",
        value="Our team monitors every trade. If any issues arise, open a support ticket and our staff will investigate and resolve the matter promptly.", inline=False)
    embed.add_field(name="Where can I report a scammer?",
        value="Open a support ticket and provide all relevant proof. Our team will handle the report and take appropriate action.", inline=False)
    embed.set_footer(text="BloxExchange Marketplace • Today")
    await interaction.response.send_message(embed=embed, view=InfoView())
...
async def cmd_tos(interaction: discord.Interaction):
    ...
    embed.add_field(name="1. Cross-Trading",
        value="Cross-trading is only allowed with server-approved middlemen. Violations will result in a warning. (3 warnings = mute)", inline=False)
    embed.add_field(name="2. Prohibited Statements",
        value="Statements like 'mm of my choice' or 'ngl' are not allowed during cross-trading and will result in warnings. (3 warnings = mute, further violations = ban)", inline=False)
    embed.add_field(name="3. Trading Locations",
        value="Cross-trading is only permitted in the #marketplace and #verified-market channels.", inline=False)
    embed.add_field(name="4. Middleman Violations",
        value="Suggesting a scam middleman or refusing to use trusted middlemen will result in an **instant ban**. If you find someone suggesting a scam server, please report them immediately in #support.", inline=False)
    embed.add_field(name="5. Illegal Trading",
        value="Trading illegal items is strictly prohibited and will result in an instant ban. This includes trading Discord Nitro, accounts, selling scripts or cheats for games, or anything else that violates Discord's Terms of Service.", inline=False)
    embed.add_field(name="6. Middleman Usage",
        value="Always use a middleman when you do a cross-trade, and ensure you follow our middleman TOS. Failure to comply may result in penalties. To use middle man go to the channel #request-mm.", inline=False)
    embed.add_field(name="7. Respectful Trading",
        value="Be kind and respectful towards all traders, especially new ones. Rude or toxic behavior may lead to warnings or bans.", inline=False)
    embed.set_footer(text="BloxExchange Marketplace • Today")
    await interaction.response.send_message(embed=embed, view=InfoView())
...
async def cmd_scam(interaction: discord.Interaction):
    embed = discord.Embed(title="🛡️ BloxExchange | Scam Awareness Guide", color=0x2b2d31)
    embed.description = (
