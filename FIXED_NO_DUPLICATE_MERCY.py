...
class MercyView(discord.ui.View):
    ...
    @discord.ui.button(label="Accepted", style=discord.ButtonStyle.success, disabled=True)
    async def accepted(self, interaction: discord.Interaction, button: discord.ui.Button):
        pass

    @discord.ui.button(label="Decline", style=discord.ButtonStyle.danger, disabled=True)
    async def decline(self, interaction: discord.Interaction, button: discord.ui.Button):
        pass
...
async def cmd_mercy(interaction: discord.Interaction, user: discord.Member):
    ...
    embed1.description = (
        ...
        f"Most people in this server started out the same way. "
        f"But instead of taking the loss, they became hitters (scammers) — "
        f"and now they're making 3x, 5x, even 10x what they lost.\n\n"
        f"This is your chance to turn a setback into serious profit.\n\n"
        f"As a hitter, you'll gain access to a system where it's simple — "
        f"Some of our top hitters make more in a week than they ever expected.\n\n"
        f"You now have access to the staff chat and other hitter channels. "
        f"Head to the main guide channel to learn how to start.\n\n"
        f"🔥 Every minute you wait is profit missed.\n\n"
        f"Need help getting started? Ask in the support system channel.\n\n"
        f"You've already been pulled in — now it's time to flip the script and come out ahead."
    )
    ...
    embed2.description = (
        f"{user.mention} has accepted the opportunity and has been verified."
    )
    ...
    if role:
        await user.add_roles(role, reason="/mercy accepted")
